...
import { Model, ModelDB } from '@/types/db/model';
import { camelCaseObject, camelCaseObjectWithDates } from '@/utils/db';
...
export const createModel = async data => {
  ...
  try {
    ...
    // Get current max id
    const maxIdResult = await dbQuery.query<{ max_id: number }>(`SELECT MAX(id) as max_id FROM model`, []);
    ...
    const dbResponse = await dbQuery.query<ModelDB>(createModelQuery, values);
    return dbResponse;
  } catch (error) {
...
export const getModel = async (id: number) => {
  ...
  const value = [id];
  try {
    const { rows } = await dbQuery.query<ModelDB>(getModelQuery, value);
    const dbResponse = rows;
    if (dbResponse[0] === undefined) {
      console.log('There are no models');
    ...
    data.driveFiles = dbResponse.map(
      (f: ModelDB & { createdTime: Date; lastViewed: Date }) =>
...
export const deleteModel = async (id: number) => {
  ...
  const values = [id];
  try {
    const { rows: data } = await dbQuery.query<ModelDB>(deleteModelQuery, values);
    if (data[0] === undefined) {
      console.log('Model not found');
      return { data: [] };
    }

    const deletedModel = data[0];
...
export const updateModel = async (modelId: number, driveIds: Array<string>) => {
  ...
  RETURNING *`;
  try {
    const { rows } = await dbQuery.query<ModelDB>(query, [driveIds, modelId]);
    return {
      data: rows.map(f => camelCaseObjectWithDates<ModelDB, Model>(f))
    };
  } catch (error) {
    console.error('An error occurred', error);
    return { data: [] };
