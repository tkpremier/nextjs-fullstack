...
import { DriveRowIdentifier } from '@/types/db/drive';
...
export const camelCaseObjectWithDates = <T extends Record<string, unknown>, R extends Record<string, unknown>>(
  obj: T = {} as T,
  dateKeys: string[] = ['createdOn', 'createdTime', 'lastViewed'],
  dateFormat: string = "MM/dd/yyyy' 'h:mm a"
): R => {
  const camelCased = camelCaseObject(obj) as R;
  return formatDateFields(camelCased, dateKeys, dateFormat) as R;
};
